...
fun LazyListScope.homeFollowTabScreen(navController: NavHostController, homeUiState: HomeUiState, viewModel: HomeViewModel) {
    item {
        Text(
    ...
    items(10) {
        Text(
            text = homeUiState.optionalCounter.toString(),
            fontSize = 80.sp,
            modifier = Modifier.clickable { viewModel.updateOptionalCount() })
    }
}